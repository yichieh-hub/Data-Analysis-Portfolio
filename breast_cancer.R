...
############### Data loading ###############
############################################
cancer_data <- read.csv("breast_cancer.csv", header = T, sep = ",", stringsAsFactors = TRUE)
...
for (i in 1:cv)
{
  ...
  for (j in 1:4)
  {
    table.KNN[cv + 1, j] <- mean(table.KNN[1:cv, j])
  }
  table.KNN
  ...
  for (j in 1:4)
  {
    table.NB[cv + 1, j] <- mean(table.NB[1:cv, j])
  }
  table.NB
  ...
  for (j in 1:4)
  {
    table.LR[cv + 1, j] <- mean(table.LR[1:cv, j])
  }
  table.LR
...
for (j in 1:4)
{
  table.ANN[cv + 1, j] <- mean(table.ANN[1:cv, j])
}
table.ANN
